...
fn apply_length_mutations(buf: &mut [u8], offset: usize) {
    ...
    match rng.gen_range(0..6) {
        ...
        4 => buf[offset] = (buf.len() as u8).wrapping_add(10),
        _ => buf[offset] = 128,
    }
}
fn apply_boundary_bypass_mutations(buf: &mut [u8], offset: usize) {
    let mut rng = thread_rng();
    match rng.gen_range(0..4) {
        0 => buf[offset] = buf[offset].wrapping_add(1),
        1 => buf[offset] = 255,
        2 => buf[offset] = 0x80,
        _ => buf[offset] = buf[offset].wrapping_sub(1),
    }
}
fn apply_index_overflow_mutations(buf: &mut [u8], offset: usize) {
    let mut rng = thread_rng();
    match rng.gen_range(0..3) {
        0 => buf[offset] = 255,
        1 => buf[offset] = 128,
        _ => buf[offset] = buf.len() as u8,
    }
}
fn apply_vulnerability_fuzz(buf: &mut [u8], offset: usize) {
    buf[offset] = thread_rng().r#gen::<u8>();
}

#[no_mangle]
pub unsafe extern "C" fn afl_custom_init(_afl: *mut c_void, _seed: u32) -> *mut MutatorState {
    let hints_path =
        std::env::var("TREESCANNER_HINTS").unwrap_or_else(|_| "hints.json".to_string());
    let map = fs::read_to_string(&hints_path)
        .ok()
        .and_then(|c| serde_json::from_str(&c).ok())
        .unwrap_or_else(|| StructuralMap::new(vec![]));
    let mut severity_weights = HashMap::new();
    for h in &map.hints {
        severity_weights.insert(h.label.clone(), weight_for(&h.severity));
    }
...
pub unsafe extern "C" fn afl_custom_fuzz(
    ...
    if !state.map.hints.is_empty() {
        let total: f32 = state
            ...
            .hints
            .iter()
            .map(|h| *state.severity_weights.get(&h.label).unwrap_or(&1.0))
            .sum();
                ...
                if let Some(offset) = hint.offset.filter(|o| *o < state.buf.len()) {
                    match hint.kind {
                        ...
                        HintKind::BoundaryCheck => {
                            apply_boundary_bypass_mutations(&mut state.buf, offset)
                        }
                        HintKind::ArrayIndex => {
                            apply_index_overflow_mutations(&mut state.buf, offset)
                        }
